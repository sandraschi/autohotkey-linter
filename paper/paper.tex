...
\textbf{Parsing layer:} An optional Lark PEG grammar parses AHK v2 syntax into an AST. When parsing fails (due to the grammar's known incompleteness), the linter falls back to per-line regex matching. This hybrid design ensures every file is analyzed regardless of parser limitations. In the current implementation this fallback triggers on the large majority of real-world files, including our own v2 fixtures, because the grammar's construct coverage remains narrow (Section~\ref{sec:limitations}); the rule layer below is consequently regex-driven for most of its detections today.
...
Total issues detected & 95,649 \\
\quad Errors (runtime failures) & 21,205 \\
\quad Warnings (style/safety) & 74,444 \\
Mean issues per KLOC & 902.7 \\
...
Across the expanded corpus, \texttt{ahk-lint} detected 903 issues per KLOC on average. Every script in the corpus contained at least one error. The per-script density ranged from 5 to 2,875 issues/KLOC, reflecting the wide variation in v1 pattern concentration across codebases. Excluding rule P001 (see below), the most prevalent genuine v1 patterns were missing \texttt{\#Requires}/\texttt{\#SingleInstance} directives, the legacy \texttt{\%var\%} variable-dereference syntax, and command-style \texttt{Gui, Add} calls. These results confirm that v1 patterns are pervasive in public AHK code and that a dedicated linter is necessary for practical v2 adoption.

Rule P001 -- omitted from the table above -- fired on 674 of 676 corpus scripts (99.7\%). It is not a v1-pattern detector: it signals that the file could not be fully parsed into an AST and was analyzed via regex fallback instead (Section~\ref{sec:limitations}). Because our grammar's syntax coverage is narrow, P001 fires on the large majority of real-world AHK files regardless of whether they use v1 or v2 syntax -- it fired on all 13 of our own hand-written v2 production fixtures as well -- so it should be read as a parser-coverage caveat on every number in this table, not as evidence about v1 prevalence specifically.
...
\textbf{Limited parser coverage:} Our Lark grammar previously contained a reduce/reduce collision in its \texttt{if\_stmt} rule (a redundant alternative that duplicated the base production) that prevented the grammar from loading \emph{at all} -- every file, v1 or v2, fell back to per-line regex matching with zero exceptions. This was fixed after being caught during the expanded-corpus evaluation (Section~\ref{sec:expanded-corpus}, rule P001) by removing the duplicate alternative and switching from LALR(1) to Earley parsing, which tolerates the grammar's remaining ambiguity. The grammar now loads and correctly parses control-flow constructs it is given, but its coverage of AHK v2 syntax is still narrow (e.g., array/object literals, ternary expressions, and several assignment forms are not modelled), so most real-world files -- including our own clean v2 fixtures -- still fall back to regex analysis for at least part of their content. The thqby LSP (9,525 lines of TypeScript) remains the only complete parser; integrating it via subprocess or porting its tokenizer state machine to Python would enable more sophisticated AST-based analyses.
