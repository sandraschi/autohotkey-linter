...
\textbf{Parsing layer:} An optional Lark PEG grammar parses AHK v2 syntax into an AST. When parsing fails (due to the grammar's known incompleteness), the linter falls back to per-line regex matching. This hybrid design ensures every file is analyzed regardless of parser limitations. As of the coverage extension described in Section~\ref{sec:limitations}, 10 of our 13 hand-written v2 production fixtures now parse successfully to an AST; the remaining 3, and the large majority of the v1 corpus (correctly, since v1 syntax is not valid v2), still use the regex fallback. The rule layer below therefore runs on a mix of AST- and regex-derived facts depending on the file.
