...
\textbf{Parsing layer:} An optional Lark PEG grammar parses AHK v2 syntax into an AST. When parsing fails (due to the grammar's known incompleteness), the linter falls back to per-line regex matching. This hybrid design ensures every file is analyzed regardless of parser limitations. As of the coverage extension described in Section~\ref{sec:limitations}, all 13 of our hand-written v2 production fixtures now parse successfully to an AST (one required fixing a genuine brace-mismatch typo in the fixture itself, not a grammar gap). The large majority of the v1 corpus still uses the regex fallback, correctly, since v1 command syntax is not valid v2 expression syntax. The rule layer below therefore runs on AST-derived facts for well-formed v2 input and regex-derived facts for v1 input, by design.
...
Total lines of code & 189,079 \\
Total issues detected & 95,645 \\
\quad Errors (runtime failures) & 21,205 \\
\quad Warnings (style/safety) & 74,440 \\
Mean issues per KLOC & 902.5 \\
Scripts with $\ge$1 error & 676 (100\%) \\
\midrule
\textbf{Top 5 v1-pattern rules (by script count)} \\
\quad S005 (missing \texttt{\#Requires}/\texttt{\#SingleInstance}) & 548 \\
\quad W008 (\texttt{\%var\%} legacy dereference syntax) & 479 \\
\quad W003 (\texttt{Gui, Add} command syntax) & 79 \\
\quad S010 (tab characters, style) & 69 \\
\quad W007 (\texttt{Gosub} label use) & 35 \\
\bottomrule
\end{tabular}
\end{table}

Across the expanded corpus, \texttt{ahk-lint} detected 902.5 issues per KLOC on average. Every script in the corpus contained at least one error. The per-script density ranged from 5 to 2,875 issues/KLOC, reflecting the wide variation in v1 pattern concentration across codebases. Excluding rule P001 (see below), the most prevalent genuine v1 patterns were missing \texttt{\#Requires}/\texttt{\#SingleInstance} directives, the legacy \texttt{\%var\%} variable-dereference syntax, and command-style \texttt{Gui, Add} calls. These results confirm that v1 patterns are pervasive in public AHK code and that a dedicated linter is necessary for practical v2 adoption.

Rule P001 -- omitted from the table above -- fired on 670 of 676 corpus scripts (99.1\%). It is not a v1-pattern detector: it signals that the file could not be fully parsed into an AST and was analyzed via regex fallback instead (Section~\ref{sec:limitations}). This is expected and correct here, not a coverage gap: v1's command-style syntax (\texttt{Random, var, 1, 10}, \texttt{StringSplit}, etc.) is not valid AHK v2 expression syntax, so a v2 grammar failing to parse it and falling back to regex is the intended behavior. After extending the grammar's v2 construct coverage (Section~\ref{sec:limitations}), P001 now fires on 0 of our 13 hand-written v2 production fixtures, down from all 13 -- so on this corpus it can now be read as a genuine v1-vs-v2 syntax signal rather than a parser-coverage artifact, at least for the syntax patterns represented in our fixtures; broader real-world v2 code may still exercise constructs the grammar does not yet model.
...
\begin{tabular}{lrr}
\toprule
\textbf{Metric} & \textbf{llama3.2:3b} & \textbf{qwen2.5-coder:32b} \\
\midrule
R1 errors & 62 & 20 \\
R2 errors & 17 & 0 \\
...
\textbf{Limited parser coverage:} Our Lark grammar previously contained a reduce/reduce collision in its \texttt{if\_stmt} rule (a redundant alternative that duplicated the base production) that prevented the grammar from loading \emph{at all} -- every file, v1 or v2, fell back to per-line regex matching with zero exceptions. This was fixed after being caught during the expanded-corpus evaluation (Section~\ref{sec:expanded-corpus}, rule P001) by removing the duplicate alternative and switching from LALR(1) to Earley parsing, which tolerates the grammar's remaining ambiguity. We then extended construct coverage directly against our v2 production fixtures: array/object literals, fat-arrow functions, static class methods, compound assignment operators, \texttt{switch}/\texttt{case}, single-quoted strings, implicit whitespace concatenation, the \texttt{??} null-coalescing operator, case-insensitive keyword matching (AHK is fully case-insensitive; the grammar had hardcoded exact-case literals), and -- the most consequential single fix -- correcting the string-escape model, which had assumed backslash escaping when AHK actually uses the backtick (\texttt{`n}, \texttt{`"}, \texttt{``}) and treats backslash as an ordinary literal character, silently breaking on any string containing a Windows path or regex pattern. This raised AST coverage on our 13 v2 production fixtures from 0/13 to 10/13. Two remaining patterns -- chained assignment (\texttt{a := b.C := d}) and inline byref defaults (\texttt{\&size := 0}) -- initially caused catastrophic Earley ambiguity blowup when added naively (single-file parse time exceeding 30 seconds), because the recursive rule was reachable from every point an expression could appear. Removing a redundant duplicate production (\texttt{prop\_assign}, which matched an input already covered by \texttt{simple\_assign} once \texttt{variable} was generalized to a full postfix chain) reduced baseline ambiguity, and re-scoping both patterns as narrowly as possible -- chained assignment restricted to \texttt{simple\_assign} only (not the rarer global/static forms), byref defaults restricted to call-argument position only (not the general expression grammar) -- resolved both without the blowup, bringing coverage to 13/13. The thirteenth fixture's remaining failure after the earlier fixes was not a grammar gap at all: the fixture file itself had two stray unmatched closing braces, a genuine typo predating this work, fixed directly. Earley parsing remains markedly slower than the LALR mode we moved away from -- successful v2 parses take roughly 2--8 seconds per file, though failed v1 parses remain fast ($<$0.1s), so the fallback path is not a performance concern; the Earley cost on genuine v2 input is tracked as future work. The thqby LSP (9,525 lines of TypeScript) remains the only complete parser; integrating it via subprocess or porting its tokenizer state machine to Python would enable more sophisticated AST-based analyses without this performance tradeoff.
